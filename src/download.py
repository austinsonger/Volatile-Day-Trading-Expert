...
def download(tickers: list, start: Union[str, int] = None, end: Union[str, int] = None, interval: str = "1d") -> dict:
    ...
    if not os.path.exists(stock_info_filename):
        # create a .csv to store stock information
        with open(stock_info_filename, 'w') as file:
            wr = csv.writer(file)
            wr.writerow(stock_info_columns)
    # load stock information file
    stock_info = pd.read_csv(stock_info_filename)
    missing_tickers = [ticker for ticker in tickers if ticker not in stock_info['SYMBOL'].values]
    missing_stock_info, na_stock_info = {}, {}
    currencies = {}
        ...
        try:
            ...
            if ticker in missing_tickers:
                ...
                try:
                    ...
                    missing_stock_info[ticker] = dict(sector=info["sector"], industry=info["industry"])
                except:
                    pass
        except:
            pass
    ...
    info = zip(list(missing_stock_info.keys()), [currencies[ticker] for ticker in missing_stock_info.keys()],
               [v['sector'] for v in missing_stock_info.values()],
               [v['industry'] for v in missing_stock_info.values()])
    with open(stock_info_filename, 'a+', newline='') as file:
        wr = csv.writer(file)
        for row in info:
            wr.writerow(row)
    stock_info = pd.read_csv(stock_info_filename).set_index("SYMBOL").to_dict(orient='index')
    ...
    currencies = [stock_info[ticker]['CURRENCY'] if ticker in stock_info else currencies[ticker] for ticker in tickers]
    unique_currencies, counts = pd.unique(currencies, return_counts=True)
    default_currency = unique_currencies[counts.argmax()]
    exchange_rates = get_exchange_rates(currencies, default_currency, data.index, start, end, interval)
    ...
    return dict(tickers=tickers,
                ...
                sectors={ticker: stock_info[ticker]['SECTOR'] if ticker in stock_info else "NA_" + ticker for ticker in
                         tickers},
                industries={ticker: stock_info[ticker]['INDUSTRY'] if ticker in stock_info else "NA_" + ticker for
                            ticker in tickers})
...
def get_exchange_rates(from_currencies: list, to_currency: str, dates: pd.Index, start: Union[str, int] = None,
    ...
    unique_currencies, counts = pd.unique(from_currencies, return_counts=True)
    tmp = {}
    if to_currency not in unique_currencies or len(unique_currencies) > 1:
        ...
        xrates = xrates.to_dict(orient='list')
    else:
        xrates = tmp
    return xrates
